...
public class IdSourceGeneratorTests
{
    ...
    [Fact]
    public void Construct_IdWithDashes()
    {
        var idWithDashes = new IdWithDashes();

        IdParser.TryParse(idWithDashes.ToString(), out var id).ShouldBe(true);

        id.ShouldBeAssignableTo<IdWithDashes>()
            .ShouldBe(id);
    }
    ...
    [Fact]
    public void Construct_NoDotsAllowed_Prefix() => Should.Throw<ArgumentException>(() => new IdWithDots());

    [Fact]
    public void Construct_NoDotsAllowed_Identifier() => Should.Throw<ArgumentException>(() => new IdWithDashes("hello.hello"));
